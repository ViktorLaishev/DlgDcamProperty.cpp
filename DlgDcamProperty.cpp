...
void CDlgDcamProperty::OnButtonCustomClicked()
{
	CString filter = _T("DAT Files (*.DAT)|*.DAT|All Files (*.*)|*.*||");
	CFileDialog fileDialog(TRUE, _T("DAT"), NULL, OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST | OFN_ALLOWMULTISELECT, filter, NULL);
	TCHAR* fileBuffer = new TCHAR[4096 * (MAX_PATH + 1)]{ 0 };
	fileDialog.m_ofn.lpstrFile = fileBuffer;
	fileDialog.m_ofn.nMaxFile = 4096 * (MAX_PATH + 1);
	...
	while (pos != NULL)
	{
		...
		CT2A ascii(filePath); // Convert CString to std::string
		filenames.emplace_back(ascii);
	}
	int roiStartRow = 1800;
	int roiEndRow = 2000;
	int roiStartCol = 890;
	int roiEndCol = 1000;

	std::vector<std::vector<BlockInfo>> maxBlocksList;
	...
	for (const std::string& filename : filenames)
	{
		...
		{
			AfxMessageBox(_T("Failed to read the file."));
			file.close();
			delete[] fileBuffer; // Clean up allocated memory
			return;
		}
		...
		BlockInfo maxBlock;
		maxBlock.average = 0;
		maxBlock.countGreater2 = 0;

		int blockRows = 7;
		int blockCols = 13;

		for (int row = roiStartRow; row <= roiEndRow - blockRows; row += blockRows)
		{
			for (int col = roiStartCol; col <= roiEndCol - blockCols; col += blockCols)
			{
				int currentEndRow = row + blockRows - 1;
				int currentEndCol = col + blockCols - 1;

				int sum = 0;
				int countGreater2 = 0;
				for (int r = row; r <= currentEndRow; r++)
				{
					for (int c = col; c <= currentEndCol; c++)
					{
						sum += data[r][c];
						if (data[r][c] > 2)
							countGreater2++;
				...
				double average = static_cast<double>(sum) / (blockRows * blockCols);

				if (average > maxBlock.average && countGreater2 > maxBlock.countGreater2)
				{
					maxBlock.startRow = row;
					maxBlock.endRow = currentEndRow;
					maxBlock.startCol = col;
					maxBlock.endCol = currentEndCol;
					maxBlock.average = average;
					maxBlock.countGreater2 = countGreater2;
				}
			}
		}

		maxBlocksList.push_back({ maxBlock });
	}

	delete[] fileBuffer; // Clean up allocated memory

	bool isMovementDetected = false;
	for (size_t i = 1; i < maxBlocksList.size(); i++)
	{
		if (maxBlocksList[i][0].startRow != maxBlocksList[i - 1][0].startRow ||
			maxBlocksList[i][0].startCol != maxBlocksList[i - 1][0].startCol ||
			maxBlocksList[i][0].endRow != maxBlocksList[i - 1][0].endRow ||
			maxBlocksList[i][0].endCol != maxBlocksList[i - 1][0].endCol)
		{
	...
	{
		TRACE(_T("Ion Moved\n"));
		AfxMessageBox(_T("Ion Moved"));
	}
	else
	{
		TRACE(_T("Ion did not move\n"));
		AfxMessageBox(_T("Ion did not move"));
	}
}
